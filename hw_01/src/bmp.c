...
static int init_pixels(bmp_img_t* img) {
  img->pixels = malloc(sizeof(char*) * img->height);
  if (img->pixels == NULL)
    return -1;
  
  for (int i = 0; i < img->height; i++) {
    img->pixels[i] = malloc(sizeof(char*) * img->width);
    if (img->pixels[i] == NULL)
      return -1;
    
    for (int j = 0; j < img->width; j++) {
      img->pixels[i][j] = malloc(sizeof(char) * 3);
      if (img->pixels[i][j] == NULL)
        return -1;
    }
  }
...
int load_bmp(FILE* bmp_file, bmp_img_t* img) {
  ...
  for (int i = img->height - 1; i >= 0; i--) {
    for (int j = 0; j < img->width; j++) {
      if (fread(img->pixels[i][j], sizeof(char), 3, bmp_file) != 3)
        return -1; 
    } 
...
void save_bmp(FILE* from_file, bmp_img_t from_bmp, FILE* to_file) {
  ...
  for (int i = from_bmp.height - 1; i >= 0; i--) {
    ...
    for (int j = 0; j < from_bmp.width; j++) 
      fwrite(from_bmp.pixels[i][j], sizeof(char), 3, to_file);
