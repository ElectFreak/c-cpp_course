...
point_node *get_point(const intrusive_node * const node) {
    return container_of(node, point_node, node);
}

static int match_point(intrusive_node *node, int x, int y) {
    point_node *pnode = get_point(node);
    return pnode->x == x && pnode->y == y;
}

static void delete_point(intrusive_list *list, intrusive_node *node) {
    remove_node(list, node);
    free(get_point(node));
}

static void print_point(intrusive_node *node) {
    point_node *pnode = get_point(node);
